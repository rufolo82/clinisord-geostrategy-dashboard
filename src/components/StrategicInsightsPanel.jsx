...
const TIPO_STYLES = {
  oportunidad: { bg: 'bg-emerald-50', border: 'border-emerald-200', text: 'text-emerald-700', badge: 'bg-emerald-100 text-emerald-700', dot: 'bg-emerald-400' },
  diferenciacion: { bg: 'bg-blue-50', border: 'border-blue-200', text: 'text-blue-700', badge: 'bg-blue-100 text-blue-700', dot: 'bg-blue-400' },
  competencia: { bg: 'bg-red-50', border: 'border-red-200', text: 'text-red-700', badge: 'bg-red-100 text-red-700', dot: 'bg-red-400' },
  consolidacion: { bg: 'bg-amber-50', border: 'border-amber-200', text: 'text-amber-700', badge: 'bg-amber-100 text-amber-700', dot: 'bg-amber-400' },
  expansion: { bg: 'bg-purple-50', border: 'border-purple-200', text: 'text-purple-700', badge: 'bg-purple-100 text-purple-700', dot: 'bg-purple-400' },
};
...
const StrategicInsightsPanel = ({ selectedLocation }) => {
  ...
  const styles = {
    header: 'flex items-center justify-between p-3 bg-gradient-to-r from-indigo-900 to-indigo-800 cursor-pointer',
    container: 'border-t border-slate-200',
  };
        ...
        <div className="divide-y divide-slate-100">
          {recommendations.length === 0 ? (
            ...
            recommendations.map((rec, i) => {
              ...
              return (
                <div key={i} className={`p-3 ${s.bg} ${i > 0 ? '' : ''}`}>
                  <div className="flex items-start gap-2">
                    <span className="text-base leading-none mt-0.5 flex-shrink-0">{rec.icono}</span>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <p className="text-xs font-semibold text-slate-800 leading-snug">{rec.titulo}</p>
                      </div>
